...
.pull-right[
```{r plot-structure-col-shape, ref.label="structure-col-shape", echo = F}
```
]
...
```{r plots-viridis, echo=F, fig.width = 14, fig.height = 7.5}
...
.pull-right[
```{r plot-structure-scale-axis-clip, ref.label="structure-scale-axis-clip", echo = F, fig.height = 3.5}
```
]
...
.pull-left[
```{r structure-coord-trans, eval = F, fig.height = 4}
...
.pull-right[
```{r plot-structure-coord-trans, ref.label="structure-coord-trans", echo = F}
```
]
...
.pull-left[
```{r structure-theme-builtin, eval = F}
ggplot(chic, aes(date, temp)) +
  geom_point() +
  theme_gray() #<<
```
...
.pull-right[
```{r plot-structure-theme-builtin, ref.label="structure-theme-builtin", echo = F}
```
]
...
.pull-left[
```{r structure-theme-builtin2, eval = F}
ggplot(chic, aes(date, temp)) +
  geom_point() +
  theme_minimal() #<<
```
...
.pull-right[
```{r plot-structure-theme-builtin2, ref.label="structure-theme-builtin2", echo = F}
```
]
...
```{r structure-theme-extensions, include = F}
if(!file.exists("./img/theme_panel.png")) {
  ...
  ggsave("./img/theme_panel.png", t, width = 20, height = 10)
}
```
...
.pull-right[
```{r plot-text-ggtext-img, ref.label="text-ggtext-img", echo = F}
```
]
